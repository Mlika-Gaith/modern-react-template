...
const Navbar = ({ toggle }: Props) => {
  ...
  return (
    <nav
      className={`h-[80px] -mt-[80px] flex justify-center items-center 
        text-[1.1rem] sticky top-0 z-[9999] max-md:transition-all max-md:delay-500 max-md:ease ${
          scrollNav ? "bg-brand-background" : "bg-transparent"
        }`}
    >
      <div className="flex justify-between h-[80px] z-1 w-full py-0 px-[24px] max-w-[1100px]">
        <LinkR
          to="/"
          className="text-[#fff] justify-self-start cursor-pointer text-[2rem] 
        ...
        <div
          className="hidden text-[1.8rem] text-[#fff] max-md:block max-md:absolute max-md:top-0 max-md:right-0
        ...
        <ul className="flex items-center list-none text-center mr-[-22px] max-md:hidden">
          {LINKS.map((link, index) => (
            <li key={link} className="h-[80px]">
              <LinkS
              to={SCROLL_TO[index]}
                className="text-[#fff] uppercase flex items-center no-underline py-0 px-[1rem] h-full 
