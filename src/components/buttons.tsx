...
export const NavButton = () => {
  return (
    <LinkR
      to="/sign-in"
      className="rounded-[50px] bg-brand-purple whitespace-nowrap py-2 px-[22px]
  ...
  return (
    <LinkS
      ...
      className={`${width} rounded-[5px] flex items-center justify-center
        ...
        ${big ? "px-12 py-4" : "px-8 py-3"} ${
        fontBig ? "text-xl" : "text-base"
      } 
        transition-colors delay-100 ease-in-out hover:bg-transparent hover:text-n-1 
        outline-none border-none cursor-pointer hover:ring-1 hover:ring-brand-purple`}
      onClick={onClick}
    >
      {text}{" "}
      {hover ? (
...
export const FormButton = ({ onClick, className, text }: FormBtnProps) => {
  return (
    <button
      className={`
        rounded-[20px] border border-brand-purple bg-brand-purple 
